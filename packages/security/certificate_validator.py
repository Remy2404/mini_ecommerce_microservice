...
def _load_pem_certificate(payload: bytes, label: str) -> x509.Certificate:
	try:
		...
		raise CertificateValidationError(f"{label} is not a valid PEM certificate") from exc
...
def _load_private_key(payload: bytes):
	try:
		...
		raise CertificateValidationError("Client private key is not a valid unencrypted PEM key") from exc
...
def _assert_certificate_time_window(certificate: x509.Certificate, now: datetime) -> None:
...
def _public_keys_match(client_certificate: x509.Certificate, private_key) -> bool:
	...
	if isinstance(cert_public_key, ed25519.Ed25519PublicKey) and isinstance(
		...
		return (
			cert_public_key.public_bytes_raw()
...
def validate_mtls_certificate_bundle(
	*,
	...
	if not _is_ca_certificate(ca_certificate):
		raise CertificateValidationError("CA certificate must include BasicConstraints(ca=True)")

	if not _has_client_auth_eku(client_certificate):
		raise CertificateValidationError("Client certificate must include CLIENT_AUTH extended key usage")
