...
def _raise_scim_error(
    *,
    ...
    elif upstream_status_code == HTTPStatus.NOT_FOUND and operation == "lookup":
    ...
    elif upstream_status_code in (HTTPStatus.UNAUTHORIZED, HTTPStatus.FORBIDDEN):
        status_code = HTTPStatus.SERVICE_UNAVAILABLE
        message = "WSO2 registration configuration error"
        error_type = f"scim_{operation}_configuration_error"
    else:
        status_code = HTTPStatus.SERVICE_UNAVAILABLE
...
def _roles_from_groups(groups: Any) -> list[str]:
    ...
    for group in groups:
        if not isinstance(group, dict):
            continue
        role_name = group.get("display") or group.get("displayName") or group.get("value")
        if role_name:
...
async def get_wso2_user_by_id(
    ...
    *,
    ...
    return _scim_user_response(payload, fallback_email=user_id)
...
async def current_wso2_user(
    ...
    *,
    ...
    return await get_wso2_user_by_id(user_id, request_id=request_id)
