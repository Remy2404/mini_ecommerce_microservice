...
from apps.product_service.app.infrastructure.storage import (
    UploadedObject,
)
...
def test_upload_image_invalid_bytes(monkeypatch) -> None:
    ...
    with patch(
        "apps.product_service.app.infrastructure.storage.image_processor.process_image_bytes",
        side_effect=ImageValidationError("Invalid image"),
    ):
        with patch(
            "apps.product_service.app.infrastructure.security.jwt_validator.validate_wso2_access_token",
            new=_fake_validate,
        ):
            with TestClient(app) as client:
                response = client.put(
...
def test_upload_image_success_triggers_cleanup(monkeypatch):
    ...
    monkeypatch.setattr(
        "apps.product_service.app.infrastructure.storage.image_processor.process_image_bytes",
        lambda data: (processed_buf, "image/webp"),
    )
    monkeypatch.setattr(
        "apps.product_service.app.infrastructure.storage.image_processor.build_image_object_key",
        lambda prefix, ct: "products/1.webp",
    )
    ...
    monkeypatch.setattr(
        "apps.product_service.app.infrastructure.storage.object_storage.get_object_storage_client",
        lambda: fake_storage,
    )
    monkeypatch.setattr(
        "apps.product_service.app.infrastructure.database.repository.update_product_image",
        AsyncMock(side_effect=fake_update_product_image),
    )
